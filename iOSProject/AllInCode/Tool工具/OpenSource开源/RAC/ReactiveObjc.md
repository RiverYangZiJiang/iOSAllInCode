# RAC受什么启发？RAC相比KVO优点-缺点-场景-RAC坑

RAC受函数式响应编程启发

RAC相比KVO优点：RAC相比KVO提供了block，代码更紧凑；并且RAC提供了一个单一的、统一的方法去处理异步的行为，包括delegate方法，blocks回调，target-action机制，notifications和KVO。提供了一种新的写代码的思维，由于RAC将Cocoa中KVO、UIKit event、delegate、selector等都增加了RAC支持，所以都不用去做很多跨函数的事。

场景：如果全工程都使用RAC来实现，对于同一个业务逻辑终于可以在同一块代码里完成了，将UI事件，逻辑处理，文件或数据库操作，异步网络请求，UI结果显示，这一大套统统用函数式编程的思路嵌套起来，进入页面时搭建好这所有的关系，用户点击后妥妥的等着这一套联系一个个的按期望的逻辑和次序触发，最后显示给用户。

RAC坑：RAC/KVO forwardInvication: 方法互相覆盖问题；RAC循环调用问题/RAC内存泄漏问题[2]
...
2.MVVM+RAC总结https://blog.csdn.net/ivolcano/article/details/90720488
...
inspired by [Functional Reactive Programming]受函数式响应编程启发. It provides APIs for
**composing and transforming streams of values**.提供组成和转换值流的API
...
If you want to learn more, we recommend these resources, roughly in order大致如下:
...
future values.不是使用就地替换和修改的可变变量，RAC提供信号以捕获现在和将来的值
...
values.通过链接、组合和响应信号，软件可以声明性地编写，而无需不断观察和更新值的代码。
...
overriding `-observeValueForKeyPath:ofObject:change:context:`.例如，文本字段可以绑定到最新时间，即使它发生了变化，而不是使用额外的代码来观察时钟并每秒更新文本字段。它的工作原理与KVO非常相似，但使用块而不是覆盖`-observeValueForKeyPath:ofObject:change:context:`。
...
code.信号也可以表示异步操作，就像[期货和承诺](http://en.wikipedia.org/wiki/Futures_and_promises)一样。这极大地简化了异步软件，包括网络代码。
...
callback blocks, target-action mechanisms, notifications, and KVO.RAC的主要优势之一是，它为处理异步行为提供了一种单一、统一的方法，包括委托方法、回调块、目标操作机制、通知和KVO。
...
But unlike KVO notifications, signals can be chained together and operated on:但与KVO通知不同，信号可以链接在一起并在以下操作：

```objc
// Only logs names that starts with "j".仅记录以“j”开头的名称。
//
...
express properties in terms of signals and operations:信号也可以用于导出状态。RAC没有根据新值观察属性和设置其他属性，而是能够用信号和操作来表达属性：

```objc
// Creates a one-way binding so that self.createEnabled will be
// true whenever self.password and self.passwordConfirmation
// are equal.创建一个单向绑定，以便每当self.password和self.passwordConfirmation相等时，self.createEnabled为true
//
// RAC() is a macro that makes the binding look nicer.RAC()是一个宏，它使绑定看起来更好。
//
...
example, they can also represent button presses:随着时间的推移，信号可以建立在任何值流上，而不仅仅是KVO。例如，它们还可以表示按钮按压：
...
changes over time.信号还可以表示计时器、其他UI事件或任何随时间变化的事件。
...
// both completed.执行2个网络操作，并在两个都完成时，将消息记录到控制台
//
...
RAC even makes it easy to bind to the result of an asynchronous operation:RAC甚至可以轻松绑定到异步操作的结果：
...
locality, and better expression of intent.从README大小的例子中很难欣赏RAC，但它可以编写状态更少、样板更少、代码位置更好、意图表达更好。

<u>For more sample code, check out [C-41][] or [GroceryList][], which are real iOS</u>
<u>apps written using ReactiveObjC</u>. Additional information about RAC can be found
in this folder.
...
### Handling asynchronous or event-driven data sources处理异步或事件驱动的数据源
...
### Chaining dependent operations依赖链的操作
...
### Parallelizing independent work平行的独立工作
...
### Simplifying collection transformations简化集合转换
